...
@section('content')
    <dl id="user">
        <dt>@lang('users.properties.name')</dt>
        <dd>{{ $user->name }}</dd>

        <dt>@lang('users.properties.username')</dt>
        <dd>{{ $user->username }}</dd>

        <dt>@lang('roles.title')</dt>
        <dd>
            <ul>
    ...
    <div class="d-flex">
        ...
        @if (Admin::user()->id !== $user->id)
            <form method="POST" action="{{ route('admin.settings.users.destroy', $user) }}">
                @csrf
                @method("DELETE")
                <button class="btn btn btn-danger" onclick="return confirm('@lang('messages.confirmations.delete')')">
                    @lang('Delete')
                </button>
            </form>
        @endif
